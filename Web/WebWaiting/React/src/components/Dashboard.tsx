...
const Dashboard: React.FC = () => {
    ...
    const [screen, setScreen] = useState<'home' | 'groups' | 'receipts' | 'profile' | 'support'>('home');
    ...
    const handleLogout = async () => {
        ...
        try {
            await logout();
            // App.tsx will handle redirect to login if you use isAuthenticated
        } catch (err) {
    ...
    // central navigation setter passed to pages that need it
    const navigate = (to: typeof screen) => setScreen(to);

    return (
        <div className="bp-wrap">
            <div className="bp-card" role="main">
                {/* Top bar (shared) */}
                ...
                {/* Router: render the requested page */}
                {screen === 'home' && (
                    <>
                        ...
                        <button className="bp-add-btn" onClick={() => alert('Add Receipts — design placeholder')}>
                            Add Receipts
                        </button>
                ...
                {screen === 'groups' && <Groups navigate={navigate} />}

                {screen === 'profile' && <Profile onRequestNavigate={navigate} />}

                {screen === 'support' && <Support navigate={navigate} />}

                {/* Bottom nav — shared between pages (keeps showing even when groups/profile rendered).
                    Pages that render themselves fully still show this nav for consistency. */}
