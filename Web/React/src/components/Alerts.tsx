  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontWeight: 800, fontSize: 20, color: 'var(--text-dark)' }}>Alerts</div>
      ...
      <div style={{ marginTop: 12, display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            <button className="btn" onClick={() => { setAlerts([]); saveAlerts([]); }}>Clear all (mock)</button>
          </div>
        </div>
          ...
          {alerts.map(a => (
            <div key={a.id} className="bp-box" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <button className="btn" onClick={() => { setEditing(a); setCreating(false); }}>Edit</button>
                <button className="btn" onClick={() => simulateTrigger(a)}>Simulate trigger</button>
                <button className="btn" onClick={() => removeAlert(a.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
                ...
                <div style={{ display:'flex', gap:8, marginTop:6, flexWrap:'wrap' }}>
                  {aiSchedule.times.map((t, idx) => (
                    <div key={idx} style={{ display:'flex', gap:6, alignItems:'center' }}>
                      ...
                      <button className="btn" onClick={()=>removeTime(idx)}>x</button>
                    </div>
                  ))}
                  <button className="btn" onClick={addTime}>Add time</button>
                </div>
              </div>
                ...
                <div style={{ display:'flex', gap:8, marginTop:6, flexDirection:'column' }}>
                  {aiSchedule.specificDates.map((d,idx) => (
                    <div key={idx} style={{ display:'flex', gap:8, alignItems:'center' }}>
                      ...
                      <button className="btn" onClick={()=>removeSpecificDate(idx)}>x</button>
                    </div>
                  ))}
                  <button className="btn" onClick={addSpecificDate}>Add date</button>
                </div>
              </div>
            </div>
          </>
