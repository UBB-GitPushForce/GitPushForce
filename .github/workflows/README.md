...
This project uses an **automated CI/CD pipeline** with GitHub Actions to ensure code quality and reliability on every push and pull request to the `main` branch. The pipeline executes 4 consecutive stages that must all pass before integration:

### 📋 Pipeline Stages

1. **Linting** - Code consistency checks using Ruff
2. **Unit Tests** - Validates functionality through unit test suite
3. **Coverage Check** - Ensures minimum 70% test coverage
4. **Integration Tests** - Full integration testing with Docker Compose and health checks

Each stage depends on the previous one, ensuring only quality code reaches production.
