...
const Profile: React.FC<ProfileProps> = ({ onRequestNavigate }) => {
  ...
  const saveEdit = () => {
    ...
    if (editingField === 'currency') {
      // update front-end currency selection only (persisted in context/localStorage)
    ...
    setEditingField(null);
    setTempValue('');
    // show inline saved message instead of alert
  ...
  const saveAll = async () => {
    // TODO: call API to save entire profile (including currency and twoFactor)
    // Example payload: { first_name, last_name, email, phone_number, currency, two_factor: profile.twoFactor }
